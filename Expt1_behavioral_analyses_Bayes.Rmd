...
m.HR = brm(HitRate ~ Group*Dimension + (1|Category) + (1|Participant),
           family = bernoulli(), data = dat.HR,cores = 4,
...
```{r, echo=FALSE}
m.HR.BNT = brm(HitRate ~ cBNT*Dimension + (1|Category) + (1|Participant), family = bernoulli(), data = dat.HR,cores = 4, iter = 4000, warmup= 1000)

summary(m.HR.edu.BNT)

hypothesis(m.HR.BNT, 'cBNT > 0')
hypothesis(m.HR.BNT, 'cBNT:DimensionLDGTHD > 0')
```
...
hypothesis(m.FA.edu.BNT, 'Education < 0')
...
### Weakly Informed Prior (i.e. not flat!)
```{r, echo=FALSE}
m.HR.WP = brm(HitRate ~ Group*Dimension + (1|Category) + (1|Participant),
                      family = bernoulli(), set_prior('normal(0,5)', class = 'b'),
                      data = dat.HR,cores = 4, iter = 4000, warmup= 1000,
           sample_prior = TRUE)

summary(m.HR)

hypothesis(m.HR.WP, 'GroupNeurotypical < 0')
hypothesis(m.HR, 'GroupParkinsons > 0')
hypothesis(m.HR, 'DimensionLDGTHD < 0')
hypothesis(m.HR,'GroupNeurotypical:DimensionLDGTHD < 0')
hypothesis(m.HR,'GroupParkinsons:DimensionLDGTHD < 0')
