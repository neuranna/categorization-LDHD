...
dat.langloc.cat <- read.csv(paste(data_dir, 'LHRHlang_langlocSN_Categorization_semperc.csv', sep=''))
dat.MDloc.MD <- read.csv(paste(data_dir, 'MD_spatialWM_spatialWM_new.csv', sep=''))
dat.MDloc.cat <- read.csv(paste(data_dir, 'MD_spatialWM_Categorization_semperc.csv', sep=''))
...
dat.langloc <- dat.langloc %>% filter(Effect %in% c('S', 'N', 'PERC', 'SEM'))
dat.langloc$Effect <- factor(dat.langloc$Effect, levels = c('S', 'N', 'PERC', 'SEM'))
dat.MDloc <- dat.MDloc %>% filter(dat.MDloc$Effect %in% c('H', 'E', 'SEM', 'PERC', 'S', 'N'))
dat.MDloc$Effect <- factor(dat.MDloc$Effect, levels = c('H', 'E', 'PERC', 'SEM', 'S', 'N'))
```
...
h = hypr(~0.5*(SEM+PERC), PERC~SEM, H~E, 0.5*(H+E)~0.5*(SEM+PERC), N~0.5*(SEM+PERC), S~0.5*(SEM+PERC), levels=c('H', 'E', 'PERC', 'SEM', 'S', 'N'))

contrasts(dat.MDloc$Effect) = contr.hypothesis(h)
colnames(attr(dat.MDloc$Effect, 'contrasts')) = c('PERC-SEM', 'H-E', 'HE-SEMPERC', 'N-SEMPERC', 'S-SEMPERC')
...
for (h in c('LH', 'RH')) {
  for (roi in unique(dat.MDloc.stats$ROI)) {
    ...
    model.df = data.frame(c('Intercept', 'PERC-SEM', 'H-E', 'HE-SEMPERC', 'N-SEMPERC', 'S-SEMPERC'),
...
y.mean = dat.MDloc %>% 
  filter(Effect %in% c('H', 'E', 'SEM', 'PERC')) %>%
  group_by(Effect, Subject, Hemisphere) %>% 
  summarize(EffectSize = mean(EffectSize))
y.mean$Effect <- factor(y.mean$Effect, levels =c('H', 'E', 'SEM', 'PERC'))
...
plot_md = ggplot(y.mean)+
  ...
  scale_fill_manual(values=c('#0830D0', '#99A5D8', '#0A730A', '#98CD92'),
                    labels=c('HardWM', 'EasyWM', 'SEM Categories', 'PERC Categories'))+
...
dat.MDloc.plot$Effect = factor(dat.MDloc.plot$Effect, levels=c('H', 'E', 'SEM', 'PERC'))
...
plot_md.ROI = ggplot(dat.MDloc.plot %>% 
  ...
  scale_fill_manual(values=c('#0830D0', '#99A5D8', '#0A730A', '#98CD92'),
                    labels=c('Hard WM', 'Easy WM', 'Semantic Cat.', 'Perceptual Cat.'))+
  scale_color_manual(values=c('#0830D0', '#99A5D8', '#0A730A', '#98CD92'))+
  theme_classic()+
...
h = hypr(~SEM, PERC~SEM, S~SEM, N~SEM, levels=c('S', 'N', 'PERC', 'SEM'))
contrasts(dat.langloc$Effect) = contr.hypothesis(h)
colnames(attr(dat.langloc$Effect, 'contrasts')) = c('PERC-SEM', 'S-SEM', 'N-SEM')
...
for (h in c('LH', 'RH')) {
  for (roi in unique(dat.langloc$ROI)) {
    ...
    model = lmer(EffectSize ~ Effect + (1|Subject), data=data.roi, REML=FALSE)
    model.df = data.frame(c('Intercept', 'PERC-SEM', 'S-SEM', 'N-SEM'),
                          summary(model)$coefficients[,'Estimate'],
                          summary(model)$coefficients[,'Pr(>|t|)']) %>% 
      mutate(ROI=roi, Hemisphere=h)
    names(model.df) = c('Regression Term', 'Beta', 'p.value', 'ROI', 'Hemisphere')
...
y.mean <- dat.langloc %>% 
  filter(Effect %in% c('S', 'N', 'SEM', 'PERC')) %>%
  group_by(Effect, Subject, Hemisphere) %>% 
  summarize(EffectSize = mean(EffectSize))
y.mean$Effect <- factor(y.mean$Effect, levels =c('S', 'N', 'SEM', 'PERC'))
...
dat.langloc.plot$Effect = factor(dat.langloc.plot$Effect, levels=c('S', 'N', 'SEM', 'PERC'))
...
plot_lang.ROI = ggplot(dat.langloc.plot)+
  ...
  scale_fill_manual(values=c('#990000', '#CA6D6D', '#0A730A', '#98CD92'),
                    labels=c('Sentences', 'Nonwords', 'Semantic Cat.', 'Perceptual Cat.'))+
  scale_color_manual(values=c('#0830D0', '#99A5D8', '#0A730A', '#98CD92'))+
  theme_classic()+
  theme(axis.text.y = element_text(size=axis_text_size), 
        axis.text.x = element_text(size=axis_text_size),
...
ggsave("Plots/lang_plot_semperc.png", height=plot_height_2rows, width=plot_width, units="cm")
```

```{r big figure 2, echo=FALSE}

(plot_md | plot_md.ROI | plot_layout(widths = c(1,4)))
ggsave("Plots/MD_plot_semperc.png", height=plot_height_2rows, width=plot_width, units="cm")
```
