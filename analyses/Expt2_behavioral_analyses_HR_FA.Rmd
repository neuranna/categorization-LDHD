...
write.csv(dat.summ, 'Tables/Supplementary/data_summary_Expt2_HR_FA.csv')
```
...
write.csv(modelcomp.HR, 'Tables/Supplementary/E2_ModelComparison_HitRate.csv')
```
...
write.csv(modelcomp.FA, 'Tables/Supplementary/E2_ModelComparison_FalseAlarm.csv')
```
...
(e2_hr |plot_spacer() |e2_fa |
   plot_layout(widths=c(1,0.1,1))) / 
  plot_spacer() / 
  (e2_hr_bnt |plot_spacer() |e2_fa_bnt |
     plot_layout(widths=c(1,0.1,1))) /
  plot_layout(heights=c(1,0.1,1))
ggsave('Plots/Supplementary/E2_results_HR_FA.png', width=25, height=20, units='cm')
```
